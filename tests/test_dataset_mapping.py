...
def test_parse_segment_from_video_maked_path(tmp_path: Path) -> None:
    ready = tmp_path / "video_maked_ready"
    index = tmp_path / "index_raw.jsonl"
    rec = {
        "frames": [
            "/home/lht/blurtrack/video_maked/S009/frames_roi/12.jpg",
            "/home/lht/blurtrack/video_maked/S009/frames_roi/13.jpg",
            "/home/lht/blurtrack/video_maked/S009/frames_roi/14.jpg",
        ],
        "target": 1,
    }
    index.write_text(json.dumps(rec) + "\n", encoding="utf-8")
    ...
    specs = parse_index(str(index), str(ready))
    assert len(specs) == 1
    assert specs[0].segment == "S009"
    assert specs[0].target_id == "000013"
    assert specs[0].target_frame_path.endswith("video_maked_ready/S009/frames_roi/000013.jpg")
